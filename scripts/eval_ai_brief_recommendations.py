...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Evaluate captured AI Brief recommendation quality offline."
    )
    parser.add_argument(
        "--entry-report",
        required=True,
        help="Entry report JSON used to derive expected ENTER candidates",
    )
    parser.add_argument(
        "--ai-brief-report",
        required=True,
        help="AI Brief report JSON compatible with sab.ai_brief.v1",
    )
    parser.add_argument(
        "--market",
        choices=["KR", "US"],
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="Minimum recommendation source-backed ratio required to pass",
    )
    parser.add_argument(
        "--now",
        type=str,
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        result = evaluate_ai_brief_recommendation_report(
            ...
            ai_brief_report_path=ns.ai_brief_report,
            market=ns.market,
