...
check (report_type in ('buy', 'sell', 'entry', 'ai-brief', 'ai-brief-skip', 'sell-ai-brief'));
...
where objects.bucket_id = 'reports'
  and objects.name ~ '^\d{4}/\d{2}/\d{4}-\d{2}-\d{2}(?:-\d+)?\.(buy|sell|entry|ai-brief|ai-brief-skip|sell-ai-brief)\.json$'
