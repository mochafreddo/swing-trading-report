...
select
  ...
  (regexp_match(objects.name, '\.(buy|sell|entry|ai-brief)\.json$'))[1] as report_type,
  ((regexp_match(objects.name, '/(\d{4}-\d{2}-\d{2})(?:-\d+)?\.(buy|sell|entry|ai-brief)\.json$'))[1])::date as report_date,
  coalesce(
    nullif(
      (regexp_match(objects.name, '/\d{4}-\d{2}-\d{2}-(\d+)\.(buy|sell|entry|ai-brief)\.json$'))[1],
      ''
    )::integer,
    0
  ) as duplicate_index,
...
where objects.bucket_id = 'reports'
  and objects.name ~ '^\d{4}/\d{2}/\d{4}-\d{2}-\d{2}(?:-\d+)?\.(buy|sell|entry|ai-brief)\.json$'
