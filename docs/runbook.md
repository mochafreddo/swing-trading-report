...
- AI Brief 수동/scheduled 실행(GitHub Actions)
  ...
  - 2026-05-20 기준 US scheduled default는 `AI_BRIEF_SOURCE_PROVIDER_US=benzinga-news`입니다. `POLYGON_API_KEY`도 비교 후보로 구성되어 있지만, `docs/ai-brief-us-source-provider-decision.md`의 blocked 항목(source coverage 미달, source-backed ratio 0.000)을 해결하기 전까지 production-ready 완료로 간주하지 않습니다. 장애 시 `AI_BRIEF_SOURCE_PROVIDER_US`를 unset하거나 다른 검증된 US provider로 바꾼 뒤 live comparison과 recommendation eval을 다시 실행합니다.
