...
- AI Brief 수동/scheduled 실행(GitHub Actions)
  ...
  - scheduled AI Brief의 운영 source 경로는 계속 `AI_BRIEF_SOURCE_API_URL` 기반 `http-json`을 사용합니다. 유료/벤더별 JSON API adapter는 SAB 내부 SDK가 아니라 이 외부 source API 경계 뒤에서 운영하는 후속 작업입니다. 로컬/live feed `sources[]` payload 품질은 `just ai-brief-source-eval --entry-report <entry.json> --source-report <sources.json>`로 점검합니다. `market=MIXED` entry report는 `--market KR|US`를 함께 지정합니다.
