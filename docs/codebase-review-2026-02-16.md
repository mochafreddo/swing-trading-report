...
- File/Location:
  - `sab/scan.py:15`
  - `tests/test_scan_types_helpers.py:3`
  - `tests/test_sell_types_and_settings.py:5`
  - `tests/test_sell_helpers.py:3`
- Why it matters:
  - 오케스트레이션 shim(`sab.scan`/`sab.sell`)이나 private helper가 테스트 계약으로 고정되면,
    내부 모듈 재구성 시 테스트가 구조에 과도하게 결합되어 리팩터링 비용이 커짐.
- Refactoring suggestion:
