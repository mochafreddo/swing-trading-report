...
- `buy|sell|entry|ai-brief|ai-brief-skip` 리포트 키 규칙과 `report_index`/`runtime_state` 계약은 현재 구현과 테스트가 따르는 기준입니다.
...
- **report**: Python 엔진(`sab scan`/`sab sell`/`sab entry`/`sab ai-brief`)이 생성하는 JSON 산출물. `ai-brief-skip`은 scheduled AI Brief runtime guard가 pipeline runner를 중단했을 때 생성하는 별도 skip 아티팩트입니다.
...
- 키 규칙(필수): `YYYY/MM/YYYY-MM-DD(.n).{buy|sell|entry|ai-brief|ai-brief-skip}.json`
...
- `report_type` TEXT NOT NULL (`buy`, `sell`, `entry`, `ai-brief`, `ai-brief-skip` 허용)
