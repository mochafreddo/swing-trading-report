...
- directional action 변경 자격은 발급 당시 원래 claim이 `action_changing=true`이고 validation 및 request/article/source/policy가 immutable issuance snapshot과 deep revalidation을 통과한 unchanged `SUPPORTED`일 때만 생깁니다.
