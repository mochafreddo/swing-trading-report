...
### 7.6 T21 주문별 체결 결과와 개별 fill 구분

실사용 체결내역은 토스증권 API를 기본 입력으로 하며 KIS 시세 사용과 분리한다. T21의
제한된 종료 주문 조회는 주문별 누적 체결 결과만 검증한다. 개별 fill ID나 정정 연결 정보가
없어도 이 조회를 검증할 수 있지만, 주문 ID를 fill ID로 바꾸거나 누적량 차이로 개별 체결을
