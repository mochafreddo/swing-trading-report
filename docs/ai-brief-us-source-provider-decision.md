...
2026-05-20 현재 US scheduled AI Brief의 repository variable은
`AI_BRIEF_SOURCE_PROVIDER_US=benzinga-news`로 설정되어 있다. `POLYGON_API_KEY`를
추가해 `polygon-news`와 비교했지만, 현재 표본에서는 어떤 US vendor provider도
source coverage exit criteria를 통과하지 못했다. 따라서 기존 scheduled default는
그대로 두되, production-ready default로 확정하지 않는다.
...
three local live comparison runs were executed against the 2026-05-19 US entry
report.
...
| Run | Artifact dir | Benzinga result | Polygon result | Status |
| --- | --- | --- | --- | --- |
...
3. Consider adding `FINNHUB_API_KEY` as a third comparison provider if
   Benzinga/Polygon coverage remains sparse.
4. Rerun US AI Brief with `model_provider=openai` and the selected provider.
