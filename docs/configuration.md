...
| `SELL_AI_BRIEF_SOURCE_PROVIDER_CHAIN_US` | no | global sell chain, then AI Brief chain fallback | `finnhub,benzinga-news` | `sab sell-ai-brief`, scheduled sell generation | US Sell AI Brief source provider chain | Each provider's secret must be configured when that provider appears in the chain. |
| `SELL_AI_BRIEF_SOURCE_PROVIDER_CHAIN_MIXED` | no | KR+US sell chain combination, then global sell chain | `naver-news,finnhub,benzinga-news` | `sab sell-ai-brief`, scheduled sell generation | MIXED Sell AI Brief source provider chain | Optional explicit override for mixed KR/US sell reports. If unset, KR then US chains are combined with duplicates removed. |
