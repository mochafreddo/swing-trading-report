...
| `TOSS_SYNC_SOURCE` | no | live Toss API | `fixture` | web `/api/holdings/toss-sync/scheduled` | QA-only holdings source override | Leave unset for production/live sync. `fixture` makes the route read the committed QA fixture only when the explicit QA guard is also enabled. |
| `TOSS_SYNC_QA_FIXTURE_ENABLED` | no | disabled | `1` | web `/api/holdings/toss-sync/scheduled` | Explicit guard for QA fixture source | Required with `TOSS_SYNC_SOURCE=fixture`; fixture mode also requires a local Supabase URL. |
| `TOSS_SYNC_JOB_TOKEN` | required for scheduled Toss auto-sync | none | `replace-with-random-local-token` | web `/api/holdings/toss-sync/scheduled`, local runner | Bearer token for local scheduled Toss sync | Secret. Store in root `.env` so Docker Compose and `scripts/toss_daily_auto_sync.sh` read the same value. |
| `TOSS_SYNC_AUTO_APPLY_ENABLED` | no | disabled | `1` | web `/api/holdings/toss-sync/scheduled` | Enables scheduled Toss auto-apply | Must be `1` to write. Any other value returns `disabled` without fetching or writing. Non-empty delete diffs quarantine missing holdings with `broker_state=not_seen_in_toss`; empty snapshot wipes return `wipe_guard_blocked`. Successful `applied`/`unchanged` writes also record the Toss freshness marker consumed by scheduled sell generation. |
| `TOSS_SYNC_REVIEWED_MAPPING_B64` | no | empty | base64-encoded private JSON | web Toss holdings sync | Approved US symbol mapping fallback | Configure only with `TOSS_SYNC_REVIEWED_MAPPING_SHA256`. The decoded strict JSON must use `toss-sync-reviewed-mapping.v1`, `review_state=APPROVED`, and `approved_by=user`; keep personal mappings uncommitted. This setting does not authorize holdings apply. |
| `TOSS_SYNC_REVIEWED_MAPPING_SHA256` | no | empty | `sha256:<64 lowercase hex>` | web Toss holdings sync | Exact approval binding for the decoded mapping bytes | Configure only with `TOSS_SYNC_REVIEWED_MAPPING_B64`. A missing pair, digest mismatch, unsupported source, duplicate symbol, or Polygon MIC/suffix mismatch fails closed. |
| `TOSS_SYNC_SESSION_DATE` | no | KST today | `2026-07-06` | local Toss runner | Fixed scheduled Toss session date | Use only for isolated smoke/replay. The runner posts this date to the web route so the freshness marker matches scheduled sell generation. |
...
| `SAB_SELL_SCHEDULE_MODE` | required for generic scheduled sell wrapper | none | `generation` | `scripts/launchd/sab-scheduled-wrapper.sh` | Explicit scheduled sell mode | `generation` runs freshness-gated sell + Sell AI Brief generation. `delivery` requires `SELL_AI_BRIEF_REPORT_PATH` and only delivers a prebuilt artifact. |
| `SAB_SESSION_DATE` | no | KST today for generation, UTC today for prebuilt delivery | `2026-07-06` | generic scheduled wrapper | Fixed scheduled session date | Use for deterministic manual smoke or replay. Normal launchd runs should leave it unset. |
...
| `OPENAI_SELL_AI_BRIEF_MODEL` | no | `OPENAI_AI_BRIEF_MODEL` fallback | `gpt-...` | `sab sell-ai-brief`, scheduled sell generation | OpenAI Sell AI Brief model | Optional sell-specific model override. |
...
| `SELL_AI_BRIEF_MODEL_TIMEOUT_SECONDS` | no | `AI_BRIEF_MODEL_TIMEOUT_SECONDS` fallback, then provider default | `60` | `sab sell-ai-brief`, scheduled sell generation | Sell AI Brief model timeout | Positive finite number. |
...
| `SELL_AI_BRIEF_SOURCE_PROVIDER_CHAIN_KR` | no | global sell chain, then AI Brief chain fallback | `naver-news` | `sab sell-ai-brief`, scheduled sell generation | KR Sell AI Brief source provider chain | Used only when no explicit source provider/source path/API URL is passed. |
| `SELL_AI_BRIEF_SOURCE_PROVIDER_CHAIN_US` | no | global sell chain, then AI Brief chain fallback | `finnhub,benzinga-news,polygon-news` | `sab sell-ai-brief`, scheduled sell generation | US Sell AI Brief source provider chain | Each provider's secret must be configured when that provider appears in the chain. |
| `SELL_AI_BRIEF_SOURCE_PROVIDER_CHAIN_MIXED` | no | KR+US sell chain combination, then global sell chain | `naver-news,finnhub,benzinga-news,polygon-news` | `sab sell-ai-brief`, scheduled sell generation | MIXED Sell AI Brief source provider chain | Optional explicit override for mixed KR/US sell reports. If unset, KR then US chains are combined with duplicates removed. |
| `SELL_AI_BRIEF_SOURCE_PROVIDER_CHAIN` | no | `AI_BRIEF_SOURCE_PROVIDER_CHAIN` fallback | `finnhub,benzinga-news` | `sab sell-ai-brief`, scheduled sell generation | Global Sell AI Brief source provider chain | Sell-specific chain override. |
