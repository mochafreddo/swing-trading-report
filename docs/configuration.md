...
| `TOSS_SYNC_AUTO_APPLY_ENABLED` | no | disabled | `1` | web `/api/holdings/toss-sync/scheduled` | Enables scheduled Toss auto-apply | Must be `1` to write. Any other value returns `disabled` without fetching or writing. Scheduled write is create/update only; delete diffs return `delete_guard_blocked` or `wipe_guard_blocked`. |
