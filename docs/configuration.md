...
| `TOSS_SYNC_SOURCE` | no | live Toss API | `fixture` | web `/api/holdings/toss-sync/scheduled` | QA-only holdings source override | Leave unset for production/live sync. `fixture` makes the route read the committed QA fixture used by `just qa-toss-sync`. |
