...
5. `/api/holdings/toss-sync` `POST`는 Toss Securities Open API에서 보유 종목을 조회해 Supabase holdings와 reconciliation을 반환하고, guarded apply를 수행합니다.
   ...
   - dry-run 응답은 `diffHash`를 포함합니다. apply 요청은 서버가 Toss/Supabase를 다시 조회해 reviewed `diffHash`가 여전히 같은지 확인하고, blocked row가 없을 때만 Supabase RPC(`replace_holdings_v1`)를 호출합니다. stale 또는 blocked apply는 `409`로 차단합니다.
