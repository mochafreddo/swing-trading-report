...
- GitHub Actions는 CI/audit/release, cleanup, manual dispatch, AI Brief monitor/fallback을 담당합니다. scheduled AI Brief는 로컬 Docker primary가 실행하고, scheduled scan/sell 생성은 marker-aware fallback 전까지 fail closed입니다. Sell AI Brief는 prebuilt `*.sell-ai-brief.json` artifact를 대상으로 한 marker-aware scheduled delivery만 현재 제공합니다.
...
| 컴포넌트 | 역할 | 주요 코드 |
|---|---|---|
| CLI 엔트리 | `scan`/`sell`/`entry`/`ai-brief`/`sell-ai-brief`/`ai-brief-scheduled`/`sell-ai-brief-scheduled`/`ai-brief-latency-probe` 서브커맨드 라우팅 | `sab/__main__.py` |
...
| Scheduled Sell AI Brief delivery | 기존 `*.sell-ai-brief.json` artifact를 검증하고, `scheduled-sell:*` marker로 upload/index-before-notify 및 notification reconciliation을 보장하는 전달 runner | `sab/scheduler/sell_ai_brief_delivery.py`, `scripts/launchd/sab-scheduled-wrapper.sh` |
...
### 4.3.2 `sell-ai-brief` 로컬/manual 생성 + `sell-ai-brief-scheduled` 전달 플로우
...
8. 로컬에서는 `SAB_UPLOAD_REPORTS=true` 또는 명시적 `sab sell-ai-brief --upload`일 때 Supabase Storage 업로드 + `report_index` upsert를 수행합니다. manual `workflow_dispatch` `sell.yml` workflow는 생성 직후 업로드를 억제하고, `scripts/eval_sell_ai_brief.py` 품질 게이트 통과 뒤 force upload합니다. `sell.yml`은 여전히 operator opt-in manual delivery이며 scheduled sell 생성 워크플로우를 의미하지 않습니다.
9. `sab sell-ai-brief-scheduled --sell-ai-brief-report <path>`는 기존 `*.sell-ai-brief.json` artifact만 전달합니다. 이 runner는 새 sell report나 Sell AI Brief를 생성하지 않고, artifact의 `report_date` 또는 명시적 `--session-date`를 기준으로 `scheduled-sell:*` runtime_state marker를 사용해 업로드/전달을 정합성 있게 재시도합니다.
10. 전달 runner는 `scheduled-sell:attempt:*` pre-lock 관측 marker를 남긴 뒤 main `scheduled-sell:lock:*`을 획득한 경우에만 artifact를 다시 읽고 `validate_sell_ai_brief_artifact(...)`로 구조/시간 계약을 검증합니다. 검증이 통과하면 Supabase Storage 업로드와 `report_index` 반영을 먼저 완료하고 `scheduled-sell:artifact:*`를 기록한 다음, `scheduled-sell:notification:claim:*`으로 Telegram 전달 단일 소유권을 잡고 `scheduled-sell:notification:sent:*`, `scheduled-sell:success:*` 순서로 마감합니다. 이미 `artifact` marker가 있고 `success`가 없으면 저장된 storage key를 사용해 notification reconciliation만 수행합니다.
11. `scripts/launchd/sab-scheduled-wrapper.sh`는 `--pipeline sell --scope MIXED`와 `SELL_AI_BRIEF_REPORT_PATH`가 함께 주어졌을 때 generic wrapper 경로에서 `python -m sab sell-ai-brief-scheduled`로 위 전달 runner를 호출합니다. 이는 prebuilt artifact delivery 전용 우회 경로이며, scheduled sell generation을 열지 않습니다.
12. `notification_text`는 Sell AI Brief artifact를 Telegram HTML rich text로 렌더링할 수 있습니다. manual `sell.yml` workflow는 업로드 후 `send_sell_ai_brief_notifications=true`일 때만 이 텍스트를 Telegram으로 전송하고, scheduled delivery runner도 동일한 HTML renderer를 사용합니다. 본문은 원본 `sell_action`, AI stance/confidence, 판단 이유, 체크리스트, source/시스템 이슈를 표시하지만 자동 체결이나 브로커 실행을 의미하지 않습니다.
