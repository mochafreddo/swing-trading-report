...
8. **2026-06 superseding note: `entry_pattern` active-position metadata**
  ...
  - broader holdings invariant는 inactive row의 `entry_pattern`을 `null`로 저장하는 것입니다. RPC가 `setof public.holdings`를 반환하므로 Add Buy 응답에는 nullable `entry_pattern` 필드가 포함됩니다.
