...
- `sell-ai-brief-generate-scheduled` is the scheduled generation path. It is currently `MIXED` only and requires a matching `toss-sync:success:MIXED:<sessionDate>` marker with status `applied` or `unchanged`; otherwise it writes blocked markers and sends only the freshness-blocked notification.
...
- Generation statuses include `dry_run`, `success_marker_skip`, `toss_freshness_missing`, `toss_freshness_stale`, `toss_freshness_invalid`, `lock_held_skip`, `lock_lost_before_upload`, `sell_report_failed`, `sell_ai_brief_failed`, `quality_gate_failed`, `upload_failed`, delegated delivery failure statuses, `delivery_failed`, `delivery_lock_held`, `completed`, and `completed_review_required`.
