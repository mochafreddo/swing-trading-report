...
| `decision-board-shadow-live` | explicit credentialed Decision Board live-shadow adapter | `decision-board` + required `--gate-manifest-sha256` | sealed snapshot metadata의 manifest hash가 다르면 `PREPARATION_INVALID`; snapshot/provider config가 없으면 `CONFIG_UNAVAILABLE`; schedule/notification 없음 |
| `decision-board-shadow-gate-validate` | frozen 20-session gate manifest 검증 | `--manifest`, `--require-approved` | sanitized proposal/approved summary와 canonical hash; invalid/approval pending은 exit 2 |
...
| `decision-board-journal-run` | one-shot runner를 local journal로 감싸기 | journal identity/policy, paired `--gate-manifest`/`--gate-manifest-sha256`, `--dry-run`, `-- <runner argv>` | bound manifest hash/slot/runtime 검증 뒤 STARTED/terminal journal observation |
