...
- `sell-ai-brief-generate-scheduled` is the scheduled generation path. It requires a matching `toss-sync:success:<scope>:<sessionDate>` marker with status `applied` or `unchanged`; otherwise it writes blocked markers and sends only the freshness-blocked notification.
- Generation claims a `scheduled-sell:lock:*` lock, runs `sab sell` with report upload suppressed, runs `sab sell-ai-brief`, evaluates the artifact with `scripts/eval_sell_ai_brief.py`, uploads the sell report only after non-`FAIL` quality, then delegates Sell AI Brief upload/Telegram reconciliation to the delivery runner.
- Generation statuses include `dry_run`, `success_marker_skip`, `toss_freshness_missing`, `toss_freshness_stale`, `toss_freshness_invalid`, `lock_held_skip`, `lock_lost_before_upload`, `sell_report_failed`, `sell_ai_brief_failed`, `quality_gate_failed`, `upload_failed`, `completed`, and `completed_review_required`.
...
- The scheduled route is for local non-browser jobs. It requires a local request and `TOSS_SYNC_JOB_TOKEN`, respects `TOSS_SYNC_AUTO_APPLY_ENABLED`, applies create/update only when no rows are blocked, and refuses delete diffs because the Toss holdings response is not treated as a proven complete account snapshot. An empty Toss snapshot that would delete existing rows returns `wipe_guard_blocked`; any other scheduled delete diff returns `delete_guard_blocked`. After `applied` or `unchanged`, it writes `toss-sync:success:MIXED:<sessionDate>` so scheduled sell generation can trust holdings freshness. If that marker write fails, the route returns `marker_failed` and the local Toss runner exits non-zero. Manual reviewed `/api/holdings/toss-sync` apply remains the path for destructive deletes.
