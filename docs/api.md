...
| `decision-board-shadow-evaluate` | 승인 manifest의 누적 진행률·품질·expected action을 read-only로 집계 | `--manifest`, `--input-ledger`, `--expected-action-ledger`, `--snapshot-dir`, `--journal-dir`, `--report-dir`, UTC `--as-of`, optional `--format evaluation|dashboard-artifact`, `--output` | sanitized canonical JSON을 stdout으로 출력하거나 지정 파일을 mode `0600`으로 atomic replace; source mutation/network/provider 호출 없음 |
...
`scripts/launchd/build_decision_board_shadow_dry_run_package.py`는 validated gate manifest의 한 session을 ENTRY/HOLDING disabled plist 두 개로 렌더링합니다. `--session`, absolute `--journal-dir`, 새 `--output-dir`가 필요하며 생성물에는 schedule/auto-start가 없습니다.
