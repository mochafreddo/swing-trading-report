...
_REQUEST_BASELINES: dict[
    int,
    tuple[weakref.ReferenceType[ClaimRequestV0], tuple[object, ...]],
] = {}
...
class ClaimValidationV0:
    ...
    def to_public_dict(self) -> dict[str, object]:
        """Serialize the exact Task 1 allowlist without polymorphic calls."""

        instrument = self.instrument
        location = self.supporting_location
        return {
            "claim_id": self.claim_id,
            "instrument": {
                "market": instrument.market,
...
_SEALED_VALIDATIONS: dict[int, weakref.ReferenceType[ClaimValidationV0]] = {}
...
def is_action_change_eligible_v0(
    ...
    *,
    ...
    request_baseline = _copy_claim_request_v0(request)
    if request_baseline is None or not request_baseline.action_changing:
        return False
    baselines = _copy_article_baselines(
        article,
        expected_source=expected_source,
    ...
    if baselines is None:
        return False
    _source, _policy, article_baseline = baselines
    if article_baseline.source.instrument != request_baseline.instrument:
        return False
    trusted = _validate_registered_claim_validation_v0(
        validation,
        request=request_baseline,
        article=article_baseline,
    )
...
def _claim_request_snapshot_v0(value: ClaimRequestV0) -> tuple[object, ...] | None:
    try:
        instrument = value.instrument
        return (
            value.claim_id,
...
def _allocate_validation_v0(
    *,
    ...
    _SEALED_VALIDATIONS[validation_id] = weakref.ref(validation, discard)
    return validation


def _validate_registered_claim_validation_v0(
    value: object,
    *,
    request: ClaimRequestV0,
    article: ArticleArtifactV0,
    ...
    if type(value) is not ClaimValidationV0:
        return None
    reference = _SEALED_VALIDATIONS.get(id(value))
