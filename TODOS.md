...
**Completion note (2026-05-23):** `AI_BRIEF_SOURCE_PROVIDER_US=finnhub` is the scheduled US default. After `FINNHUB_API_KEY` was configured, three live comparison captures against `reports/2026-05-20.entry.json` showed Finnhub covering all 3 eligible US tickers while Benzinga returned 0 sources and Polygon covered only `BABA.NYS` or hit HTTP 429. Finnhub source eval and OpenAI recommendation eval completed with `WARN` diagnostics but no `FAIL`, source coverage 1.000, source-backed recommendation ratio 1.000, and `system_issue_count=0`. See `docs/ai-brief-us-source-provider-decision.md`.

**Follow-up:** Persist runtime-guard-skipped scheduled runs as a Reports artifact only if owner-facing skip history becomes useful. This PR intentionally keeps skipped runtime guard output in Telegram/Actions summary only; a future slice should start from `.github/workflows/ai-brief.yml` guard outputs and define a separate artifact state before adding Storage/report_index writes.
...
- 2026-05-22: Quiet Desk Assistant AI Brief state slice - writer-enforced `brief_state`/`brief_reason`, Telegram rest/final/needs-review copy, skipped scheduled-run message path, Reports detail fallback, and focused Python/workflow/Web coverage.
- 2026-05-23: Phase 2 US provider completion slice - Finnhub selected and verified as US scheduled source provider, GitHub variable updated, Benzinga/Polygon failure modes documented, source/recommendation eval evidence recorded, and operational docs refreshed.
