...
- 2026-06-22: Documented and regression-tested the intentional `sma_ema_hybrid` volume confirmation distinction. `swing_high_breakout` continues to use the pre-breakout average, while `trend_pullback_bounce` and `rsi_oversold_reversal` intentionally use a signal-inclusive rolling average for confirmation.
