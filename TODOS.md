...
**Current blocker (2026-05-21):** `AI_BRIEF_SOURCE_PROVIDER_US=benzinga-news` remains the scheduled US default, but Phase 2 US exit criteria are not complete. A 2026-05-21 rerun with Benzinga and Polygon produced three more live comparison captures against `reports/2026-05-20.entry.json`; Benzinga returned 0 usable sources and Polygon again covered only `BABA.NYS` out of 3 eligible US tickers. Source eval still fails with `source_coverage_below_threshold`, and OpenAI recommendation eval was not rerun because no provider passed source eval. Smallest next action: configure `FINNHUB_API_KEY` or wait for a changed US candidate set, then rerun live comparison. See `docs/ai-brief-us-source-provider-decision.md`.
