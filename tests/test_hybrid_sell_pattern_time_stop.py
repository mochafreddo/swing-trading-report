...
def _time_stop_candles() -> list[dict[str, object]]:
    return [
        {
            "date": "20250106",
            "open": 100.0,
...
def test_hybrid_sell_uses_pattern_specific_time_stop_before_global_limit(
    ...
    result = evaluate_sell_signals_hybrid(
        "AAPL.NAS",
        cast(list[dict[str, float]], _time_stop_candles()),
        holding,
        settings,
    )
...
def test_hybrid_sell_pattern_time_stop_requires_structured_pattern(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path,
) -> None:
    _patch_hold_indicators(monkeypatch)
